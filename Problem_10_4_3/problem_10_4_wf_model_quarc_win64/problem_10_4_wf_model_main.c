...
typedef RT_MODEL_problem_10_4_wf_mode_T RT_MODEL;

/*====================*
 * External functions *
 *====================*/
EXTERN RT_MODEL * problem_10_4_wf_model(void);
  ...
  _do_assertion(const char * expression, const char * file_name, int line_number)
{
  ...
  rtmSetErrorStatus(problem_10_4_wf_model_M, GBLbuf.message);
}
  ...
  control_c_handler(int signal_number)
{
  ...
  GBLbuf.stopExecutionFlag = 1;
  rtExtModeStart();
  hil_task_stop(problem_10_4_wf_model_DW.HILReadEncoderTimebase_Task)
    ;
}
  ...
  main(int argc, char * argv[])
{
  ...
  if (parseError) {
    (void) fprintf(stderr,
                   "\nUsage: problem_10_4_wf_model -option1 val1 -option2 val2 -option3 ...\n\n");
    (void) fprintf(stderr,
  ...
  if (rtExtModeQuarcStartup(rtmGetRTWExtModeInfo(S),
    ...
    if (rtmGetErrorStatus(S) == NULL) {
      ...
      result = hil_task_start
        (problem_10_4_wf_model_DW.HILReadEncoderTimebase_Task, (t_clock)
         problem_10_4_wf_model_P.HILReadEncoderTimebase_clock, 500.0, -1)
        ;
      if (result == 0) {
